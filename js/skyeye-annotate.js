$(function() {
	...
	var skyeyeAnnotate = function(ele, opt) {
		this.defaults = { //defaults 是我们设置的默认参数。
			id: null,
			width: '260', //批注默认宽度
			userName: 'admin', //添加批注的人
			userImage: 'images/no-userphoto.png', //添加批注人的logo
			data: [], //默认回显的数据
			selectedElem: null, //当前选中的文本对象
			addIconSrc: 'images/add-icon.png', //新增批注的图标
	...
	skyeyeAnnotate.prototype = {
		...
		create: function() {
			...
			$this.mouseup(function(e) {
				...
				var addIconSrc = 'images/add-icon.png';
		...
		resetAddAnnotateBox: function() {
			...
			if(!setting.whetherLoadData) {
				...
				$("#" + annotateId).on("click",'.close',function(e) {
					console.log(e)
					if(e.target.classList.contains('close')) {
						var annotateId =	$(e.target).attr('annoid')
						$("#" + annotateId).find('.show-editor').html('')
						_op.resetData()
						$("#" + annotateId).remove()
		...
		getNowSkyeyeAnnoBox: function(params){
			return '<div class="new-skyeye-anno-box" data-text="' + params.selectText + '" data-zIndex="99999" startAdd="' + params.startAdd 
						...
						+ ' insId="' + params.insId + '" style="z-index: 99999">' +
						'<div class="anno-title">' +
							'<img src="' + params.userImage + '" class="anno-user-img"/>' +
							'<font class="anno-user-name">' + params.userName + '</font>' +
							'<font class="anno-time">' + params.createDate + '</font> <i onclick="close(this,'+params.annoId+')" class="close el-icon-error" annoid="' + params.annoId + '"/>' +
						'</div>' +
